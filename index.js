...
// Ride Schema for ride sharing (public transport rides)
...
// API endpoint to search rides by start and destination
app.get("/api/rides", async (req, res) => {
  try {
    ...
    // Use regex for case-insensitive exact match on start and destination
    const rides = await Ride.find({
      currentLocation: { $regex: new RegExp("^" + start.trim() + "$", "i") },
      destination: { $regex: new RegExp("^" + destination.trim() + "$", "i") }
    });
    res.json(rides);
  } catch (err) {
